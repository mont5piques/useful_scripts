...
# Proxy params
if len(sys.argv) < 2:
    print 'Usage : tcp_proxy local_port:destination_host:destination_port'
    sys.exit(1)

if len(sys.argv[1].split(':')) != 3:
    print 'Argument error'

(from_port, dest_host, dest_port) = sys.argv[1].split(':')
...
class ProxyServer(protocol.Protocol):
    def connectionMade(self):
        ...
        reactor.connectTCP(dest_host, dest_port, factory)
...
if __name__ == "__main__":
    ...
    reactor.listenTCP(from_port, factory) #, interface="0.0.0.0")
    reactor.run()
